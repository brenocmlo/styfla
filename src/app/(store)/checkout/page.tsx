  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (paymentMethod === 'PIX') {
        ...
        // Se Stripe real estiver configurado no frontend e backend, confirma com o cartão
        if (isStripeClientConfigured && stripe && elements && !data.isMock && data.clientSecret) {
          ...
          if (cardElement) {
            const { error: stripeErr } = await stripe.confirmCardPayment(data.clientSecret, {
              payment_method: {
                card: cardElement,
                billing_details: {
                  name: cardHolderName.trim() || formData.name.trim(),
            ...
            if (stripeErr) {
              throw new Error(stripeErr.message || 'Pagamento recusado pela operadora do cartão.');
            }
          }
        }
