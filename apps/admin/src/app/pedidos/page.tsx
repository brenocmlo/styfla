...
export default function AdminOrdersPage() {
  ...
  const handleDispatchOrder = (orderId: string) => {
    const code = inputTracking || `BR${Math.floor(100000000 + Math.random() * 900000000)}AA`;
    setOrders(
      orders.map((o) =>
        o.id === orderId
          ? { ...o, status: 'SHIPPED', trackingCode: code }
          : o
      )
    );
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-xs text-zinc-400 mt-1">
            Controle de pagamentos, picking list e despacho com rastreio integrado.
          </p>
        </div>
      </div>
              ...
              <tr key={order.id} className="hover:bg-white/[0.02]">
                ...
                <td className="py-3.5 text-right">
                  {order.status === 'PAID' ? (
                    ...
                    <button
                      onClick={() => setSelectedOrder(order)}
                      className="px-2.5 py-1 rounded bg-zinc-800 text-zinc-300 text-[10px] hover:bg-zinc-700 transition-colors"
                    >
                      Detalhes
                    </button>
                  )}
          ...
          <div className="relative w-full max-w-xl bg-[#16181D] border border-white/10 rounded-2xl p-6 shadow-2xl text-[#F4F4F6] space-y-5">
            ...
            {selectedOrder.status === 'PAID' && (
              <div className="space-y-2 pt-2">
                ...
                <div className="flex gap-2">
                  <input
                    type="text"
                    placeholder="Ex: BR948271034AA (ou deixe em branco para gerar auto)"
                    value={inputTracking}
                    onChange={(e) => setInputTracking(e.target.value)}
                    className="flex-1 px-3 py-2 text-xs rounded bg-zinc-900 border border-white/10 text-white font-mono"
                  />
                  <Button
                    variant="primary"
                    size="sm"
            ...
            <div className="pt-2 flex justify-between items-center text-xs">
              ...
              <button
                ...
                className="px-4 py-2 rounded bg-zinc-800 text-white text-xs font-bold"
              >
                Fechar
              </button>
            </div>
